...
namespace {
    ...
    // 固定后继顺序：URDL（Up, Right, Down, Left）
    ...
    inline bool applyMove(const string& s, char mv, string& out){
        ...
        int r = z / 3, c = z % 3;
        switch(mv){
            case 'U': if (r == 0) return false; swap(out[z], out[z-3]); return true;
            case 'R': if (c == 2) return false; swap(out[z], out[z+1]); return true;
            case 'D': if (r == 2) return false; swap(out[z], out[z+3]); return true;
            case 'L': if (c == 0) return false; swap(out[z], out[z-1]); return true;
        }
        return false;
    }

    // 计算启发式（注意：Misplaced Tiles 不计 0）
    int computeH(const string& s, const string& goal, heuristicFunction hf){
        if (hf == misplacedTiles){
            ...
            int sum = 0;
            int posGoal[10];
            for (int i=0;i<9;i++) posGoal[goal[i]-'0'] = i; // value→index
            for (int i=0;i<9;i++){
                int v = s[i]-'0';
                if (v == 0) continue;
                int gi = posGoal[v];
    ...
    // “最小堆”比较器：f 小者优先；若 f 相等，g 大者优先（同 f 取大 g）
    struct WorseByFG {
        const vector<Node>* nodes;
        bool operator()(int a, int b) const {
            const Node& A = (*nodes)[a];
            const Node& B = (*nodes)[b];
            if (A.f != B.f) return A.f > B.f; // larger f is worse
                  ...
                  int &numOfLocalLoopsAvoided, int &numOfAttemptedNodeReExpansions)
{
    ...
    vector<Node> nodes; nodes.reserve(100000);
    nodes.push_back(Node{initialState, 0, 0, 0, 0, -1});

    vector<int> heap; heap.reserve(100000);
    WorseByFG cmp{&nodes};
    heap.push_back(0);
    make_heap(heap.begin(), heap.end(), cmp);

    unordered_set<string> closed; closed.reserve(100003);
    unordered_map<string,int> openIndex; openIndex.emplace(initialState, 0);

    string child; int goalIdx = -1;

    while(!heap.empty()){
        pop_heap(heap.begin(), heap.end(), cmp);
        int u = heap.back(); heap.pop_back();
        openIndex.erase(nodes[u].s);

        if (nodes[u].s == goalState){ goalIdx = u; break; }

        numOfStateExpansions++;
        closed.insert(nodes[u].s);

        for(char mv : MOVES){ // URDL
            if (!applyMove(nodes[u].s, mv, child)) continue;

            // 避免本地回环：不回到父节点
            if (nodes[u].parent != -1 && child == nodes[nodes[u].parent].s){
                numOfLocalLoopsAvoided++; continue;
            }
            // Strict Expanded List：已展开过的跳过
            if (closed.count(child)){
                numOfAttemptedNodeReExpansions++; continue;
            }
            // Strict：不做 decrease-key / reopen
            if (openIndex.count(child)) continue;

            Node v;
            v.s = child;
            v.g = nodes[u].g + 1; // 每步代价=1
            v.h = 0;
            v.f = v.g;
            v.move = mv;
            v.parent = u;
            ...
            if ((int)heap.size() > maxQLength) maxQLength = (int)heap.size();
        }
    }
